    ...
    <div className="shell">
      ...
      <aside className="sidebar">
        <div className="brand">
          ...
          <div>
            ...
            <div className="brand-tag">VEILLE · IDF</div>
          </div>
        </div>

        <nav className="nav">
          <div className="nav-group eyebrow" style={{ padding: '10px 13px 4px' }}>Navigation</div>
          ...
          <div className="nav-group eyebrow" style={{ padding: '18px 13px 4px' }}>Recherches</div>
          {SAVED.map((s, i) => (
            <button key={i} className="nav-item" onClick={() => goSearch(s.q)} title={`Filtrer : ${s.label}`}>
        ...
        <div className="sidebar-foot">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
              ...
              {live ? 'Données pipeline' : 'Mode démo'}
            </span>
          </div>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div style={{ flex: 1 }} />
          <button className="iconbtn iconbtn-sm" aria-label="Rafraîchir" title="Rafraîchir" onClick={() => window.location.reload()}><RefreshCw size={15} /></button>
        </header>
