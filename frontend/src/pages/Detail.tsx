...
export default function Detail({
  ...
  const elevatorVal = l.elevator === true ? t('Oui') : l.elevator === false ? t('Non') : 'n.c.'
  const dec = decotePct(l)
  const totalW = Object.values(data.WEIGHTS).reduce((a, b) => a + b, 0)
  const axes = data.CRITERIA.map((c) => ({ short: t(c.short), value: l.crit[c.key] ?? 0 }))
  ...
  return (
    <div className="page" ref={revealRef}>
      ...
      <div className="anim d1" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <IconButton label={t('Retour')} onClick={onBack}><ChevronLeft size={18} /></IconButton>
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 5, fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>
            ...
            <span style={{ color: 'var(--text-faint)' }}>· {t('détectée il y a')} {fmtAgo(l.freshMin)}</span>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <IconButton size="sm" label={t('Précédent')} disabled={!prev} onClick={() => prev && onOpen(prev)}><ChevronLeft size={15} /></IconButton>
          <span className="mono" style={{ fontSize: 'var(--text-xs)', color: 'var(--text-faint)', minWidth: 44, textAlign: 'center' }}>{idx + 1} / {all.length}</span>
          <IconButton size="sm" label={t('Suivant')} disabled={!next} onClick={() => next && onOpen(next)}><ChevronRight size={15} /></IconButton>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Button variant="ghost" leftIcon={<Eye size={15} />} onClick={() => window.open(isRealUrl ? l.url : searchUrl, '_blank', 'noopener')}>{isRealUrl ? t("Voir l'annonce") : t('Annonces similaires')}</Button>
          <Button variant={fav ? 'ghost' : 'gold'} leftIcon={<Star size={15} fill={fav ? 'var(--gold-400)' : 'none'} />} onClick={() => toggleFav(l.id)}>{fav ? t('Suivi ✓') : t('Suivre')}</Button>
        </div>
      </div>
      ...
      <div className="detail-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }} className="anim d2">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(104px,1fr))', gap: 10 }}>
            <Fact icon={<Ruler size={12} />} label={t('Surface')} value={`${l.surface} m²`} />
            <Fact icon={<Grid3x3 size={12} />} label={t('Pièces')} value={`${l.rooms}`} />
            <Fact icon={<Layers size={12} />} label={t('Étage')} value={floorVal} />
            <Fact icon={<Maximize2 size={12} />} label={t('Balcon')} value={l.balcon ? t('Oui') : '-'} />
            <Fact icon={<Building2 size={12} />} label={t('Ascenseur')} value={elevatorVal} />
            <Fact icon={<Leaf size={12} />} label="DPE" value={l.dpe} />
          </div>

          <Panel title={t('Transports & accessibilité')} subtitle={t('Temps de marche · isochrone 15 min')} icon={<Train size={16} />}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 150px', gap: 16, alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                {l.metro.length === 0 && <span style={{ color: 'var(--text-faint)', fontSize: 'var(--text-sm)' }}>{t('Aucune gare renseignée.')}</span>}
                {l.metro.map((m, i) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }} className="anim d3">
          <Card glowGold={l.score >= 80} style={{ padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <ScoreGauge value={l.score} size={92} thickness={7} label={t(scoreLabel(l.score))} />
              <div style={{ flex: 1 }}>
                <div className="eyebrow" style={{ marginBottom: 6 }}>{t('Prix de vente')}</div>
          ...
          <Panel
            title={t("Plan d'action")}
            subtitle={plan.source === 'engine' ? t('Moteur de scoring · reco chiffrée') : t('Estimation · stratégie de négociation')}
            icon={<Target size={16} />} className="card-glow-gold"
            actions={<Badge tone={plan.source === 'engine' ? 'brand' : 'neutral'} dot={plan.source === 'engine'}>{plan.source === 'engine' ? t('moteur') : t('estimation')}</Badge>}
          >
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 10, marginBottom: 14 }}>
              <div>
                <div className="eyebrow" style={{ marginBottom: 5 }}>{t("Offre d'attaque suggérée")}</div>
              ...
              {plan.ecartOffre > 0 && (
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text-faint)' }}>{t('vs prix affiché')} · −{plan.ecartPct.toFixed(0)} %</div>
                </div>
              )}
            </div>

            <div className="eyebrow" style={{ marginBottom: 2 }}>{t('Coût de revient complet')}</div>
            <CostRow label={t('Prix affiché')} value={`${fmtEur(l.price)} €`} />
            <CostRow label={`${t('Travaux estimés')}${plan.travauxPpm2 ? ` (${plan.travauxPpm2} €/m²)` : ''}`} value={plan.travaux ? `+ ${fmtEur(plan.travaux)} €` : '-'} accent={plan.travaux > 0} />
            <CostRow label={t('Frais de notaire (7,5 %)')} value={`+ ${fmtEur(plan.fraisNotaire)} €`} />
            <CostRow label={t('Coût de revient')} value={`${fmtEur(plan.coutComplet)} €`} total />

            <div className="eyebrow" style={{ margin: '14px 0 8px' }}>{t('Leviers de négociation')}</div>
            ...
            {(plan.defenseMinutes != null || plan.netYield != null) && (
              <div style={{ display: 'flex', gap: 22, marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-subtle)' }}>
                {plan.defenseMinutes != null && (
                  <div>
                    <div className="eyebrow" style={{ marginBottom: 4 }}>{t('La Défense')}</div>
                ...
                {plan.netYield != null && (
                  <div>
                    <div className="eyebrow" style={{ marginBottom: 4 }}>{t('Rendement net est.')}</div>
          ...
          <Panel title={t('Comparaison au marché')} subtitle={`${t('Médiane quartier')} ${l.quartier} (DVF)`} icon={<TrendingDown size={16} />}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[{ k: t('Cette annonce'), v: l.ppm2, c: 'var(--brand-500)' }, { k: `${t('Médiane')} ${l.quartier}`, v: l.marketPpm2, c: 'var(--text-faint)' }].map((row, i) => {
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 2, padding: '9px 11px', background: marketDelta > 0 ? 'var(--success-soft)' : 'var(--danger-soft)', borderRadius: 'var(--radius-md)' }}>
                ...
                <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-default)' }}>
                  {marketDelta > 0
                    ? <><b style={{ color: 'var(--success-500)' }}>{fmtEur(Math.abs(marketDelta * l.surface))} €</b> {t('sous le marché estimé')}</>
                    : <><b style={{ color: 'var(--danger-500)' }}>{fmtEur(Math.abs(marketDelta * l.surface))} €</b> {t('au-dessus du marché')}</>}
                </span>
              </div>
            </div>
          </Panel>

          <Panel title={t('Justification du score')} subtitle={t('score = Σ ( critère × poids )')} icon={<Gauge size={16} />}>
            ...
            <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 6 }}>
              {data.CRITERIA.map((c) => (
                <Contribution key={c.key} short={t(c.short)} icon={c.icon} accent={c.accent} weight={data.WEIGHTS[c.key] ?? 0} value={l.crit[c.key] ?? 0} totalW={totalW} />
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-default)', marginTop: 6, paddingTop: 10 }}>
                <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--text-default)' }}>{t('Score pondéré total')}</span>
      ...
      {lb >= 0 && photos[lb] && (
        <div onClick={() => setLb(-1)} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(8,7,10,0.82)', backdropFilter: 'blur(20px) saturate(130%)', WebkitBackdropFilter: 'blur(20px) saturate(130%)', display: 'grid', placeItems: 'center', padding: 48 }}>
          ...
          {photos.length > 1 && <>
            <button onClick={(e) => { e.stopPropagation(); setLb((lb - 1 + photos.length) % photos.length) }} aria-label={t('Précédent')} style={{ position: 'absolute', left: 24, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, borderRadius: 999, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', border: '1px solid var(--border-default)', color: 'var(--text-strong)', cursor: 'pointer', fontSize: 24 }}>‹</button>
            <button onClick={(e) => { e.stopPropagation(); setLb((lb + 1) % photos.length) }} aria-label={t('Suivant')} style={{ position: 'absolute', right: 24, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, borderRadius: 999, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', border: '1px solid var(--border-default)', color: 'var(--text-strong)', cursor: 'pointer', fontSize: 24 }}>›</button>
          </>}
