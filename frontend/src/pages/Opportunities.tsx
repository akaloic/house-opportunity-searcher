    ...
    <div className="page" ref={revealRef}>
      <div className="section-head anim d1" style={{ marginBottom: 18 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            ...
            <Badge tone="info">{filtered.length} annonces</Badge>
            {q && <Badge tone="neutral">« {query} »</Badge>}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <SortTab id="score" label="Score" />
          <SortTab id="frais" label="Récent" />
          <SortTab id="fav" label="Favoris" count={filtered.length && sort === 'fav' ? undefined : data.LISTINGS.filter((l) => favs.has(l.id)).length} />
        </div>
      </div>

      <div className="split anim d2">
        <Panel title="Cartographie" subtitle="Pins réels colorés par score" icon={<MapIcon size={16} />} noPadding bodyStyle={{ padding: 10 }} style={{ minHeight: 0 }}>
          <Suspense fallback={<div style={{ height: 560, display: 'grid', placeItems: 'center', color: 'var(--text-faint)' }}>Chargement de la carte…</div>}>
            <MapPanel listings={filtered} selectedId={sel} onSelect={open} height={560} />
          </Suspense>
        </Panel>

        <Panel title="Flux de pépites" subtitle="Clic → fiche détail" icon={<Activity size={16} />} noPadding style={{ minHeight: 0 }}>
          <div style={{ maxHeight: 560, overflow: 'auto' }}>
            {filtered.length === 0
              ? <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--text-faint)', fontSize: 'var(--text-sm)' }}>
                  {sort === 'fav' ? 'Aucun favori pour l\'instant. Cliquez sur ★ pour suivre un bien.' : `Aucune annonce ne correspond${q ? ` à « ${query} »` : ''}.`}
                </div>
      ...
      <div className="section-gap anim d3">
        <div className="section-head">
          <div className="section-title" style={{ fontSize: 'var(--text-lg)' }}><Building2 size={17} style={{ verticalAlign: '-3px', marginRight: 7 }} />Toutes les annonces</div>
        </div>
        {filtered.length === 0
          ? <Panel><div style={{ textAlign: 'center', color: 'var(--text-faint)', padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}><Search size={22} />Aucun résultat.</div></Panel>
